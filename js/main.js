...
import { buildFingerprint, buildTrajectory } from './features.js?v=33';
import { DensityRenderer } from './density.js?v=33';
import { PALETTES, buildLUT, customRamp, hexToRgb } from './palettes.js?v=33';
import { exportCanvas, exportStrandSVG, framePlan, exportMP4, loopsForDuration } from './exporter.js?v=33';
import { motionParams, displacePoint } from './motion.js?v=33';
import { LiveConductor } from './live.js?v=33';
...
function regenerate() {
  ...
  try {
    if (!worker) worker = new Worker('js/worker.js?v=33', { type: 'module' });
    worker.onmessage = (e) => {
...
async function fallbackGenerate(onResult) {
  const { generate } = await import('./generators/index.js?v=33');
...
function workerGenerate(fingerprint, params) {
  return new Promise((resolve) => {
    try {
      if (!liveWorker) liveWorker = new Worker('js/worker.js?v=33', { type: 'module' });
