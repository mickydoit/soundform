...
import { selectRingSubset } from './strands.js?v=45';
...
function regenerate() {
  ...
  try {
    if (!worker) worker = new Worker('js/worker.js?v=45', { type: 'module' });
    worker.onmessage = (e) => {
...
async function fallbackGenerate(onResult) {
  const { generate } = await import('./generators/index.js?v=45');
...
function workerGenerate(fingerprint, params) {
  return new Promise((resolve) => {
    try {
      if (!liveWorker) liveWorker = new Worker('js/worker.js?v=45', { type: 'module' });
